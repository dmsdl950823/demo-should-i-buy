...
## 실행 전 준비: 역할별 모델 설정
상태: TODO — 계획에 추가됨, 실제 설정 변경 및 실행 검증은 아직 하지 않음.
...
- [ ] 실행 환경이 지원하는 프로젝트별 에이전트 설정 형식과 모델 가용성을 확인한다. 기존 설정을 읽고 보존하며 사용자 전역 기본 모델은 변경하지 않는다.
- [ ] 역할별 에이전트 설정에 모델과 추론 수준을 함께 지정하고 해당 action 문서를 작업 지침으로 연결한다. MD 파일을 읽는 것만으로 모델이 바뀐다고 가정하지 않는다.
- [ ] 파일 기반 설정이 현재 실행 환경에서 지원되지 않으면 에이전트 생성 시 명시적으로 모델·추론 수준을 전달한다. 지정 모델을 사용할 수 없다면 막힘을 기록하고 임의로 다른 모델로 대체하지 않는다.
- [ ] Plan → Builder → Reviewer 순서로 실행되도록 작업 전달 규칙을 설정한다. 메인 에이전트는 통합을 맡으며 역할을 실제 지정 모델로 실행한다. 실행 중인 메인 모델이 MD 지시만으로 전환된다고 보고하지 않는다.
- [ ] Reviewer는 Builder와 별도 에이전트로 실행하고 계획·실제 diff·검증 결과를 전달한다. Builder의 자기평가만으로 리뷰를 대체하지 않는다.
- [ ] 전체 대화 이력을 상속하면 모델 변경이 제한되는 환경에서는 필요한 문서와 작업 요약만 전달해 별도 컨텍스트로 실행한다.
- [ ] 실제 실행에서 확인 가능한 모델·추론 설정과 역할별 전달을 검증하고 결과를 기록한다. 설정 파일 작성만으로 적용 검증 완료라고 표시하지 않는다.

완료 기준: 세 역할의 모델과 추론 수준이 지정되고, 실행 환경에서 해당 설정의 적용을 확인한다. 이 작업은 Jev 키 없이 준비할 수 있지만 앱 구현은 기존 00단계 실제 호출 성공 조건을 따른다.
