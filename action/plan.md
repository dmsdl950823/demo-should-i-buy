...
## 단계별 커밋 규칙
- [x] 각 TODO 단계(실행 전 모델 설정, 00~06 및 07의 실제 산출물)는 구현·필수 검사·리뷰 수정이 끝나면 해당 단계의 변경만 별도 로컬 커밋으로 남긴다. 하나의 커밋에 여러 단계를 묶지 않는다.
- [x] 커밋 전 `git status`와 staged diff를 확인하고, 비밀 파일·키·빌드 산출물·다른 단계의 미완료 변경을 제외한다. 기존 사용자의 미커밋 변경은 임의로 포함하지 않는다.
- [x] 커밋 메시지는 단계와 결과를 구분할 수 있게 작성한다. 예: `feat(02): 구매 입력 규격과 예시 추가`.
- [x] 커밋 SHA, 실행한 검사와 리뷰 판정을 builder/reviewer 기록 및 최종 보고에 남긴다. 다음 단계에서 문제가 생기면 해당 커밋부터 비교·재현할 수 있게 한다.
- [x] BLOCKED 또는 테스트·리뷰 미통과 단계는 완료 커밋으로 표시하지 않는다. 이미 검증된 독립 산출물이 있다면 진행 중 체크포인트 커밋임을 명확히 표시한다.
- [x] 로컬 커밋은 단계마다 수행하되 원격 push와 배포는 별도 작업으로 취급한다.
...
## 04. 단일 메인 화면
상태: DONE — 단일 열 화면, 실제 Jev 결과 흐름과 독립 리뷰 PASS
선행 조건: 02; 03과 병렬 가능
담당: UI

- [x] 제목 “이거 지금 사도 될까?”와 짧은 입력 안내를 구성한다.
- [x] 제품명·가격·여유자금·현재 제품 상태·사용 빈도·필요도 입력을 제공한다.
- [x] 5개 프리셋으로 입력을 채울 수 있게 한다. 실제 API 결과가 아닌 예시 입력임을 표시한다.
- [x] 초기/입력 오류/loading/성공/API 오류 상태를 구현한다.
- [x] 요청 중 중복 제출을 막고 오류 후 입력을 유지하며 재시도할 수 있게 한다.
- [x] 입력 변경 시 이전 결과를 지우거나 오래된 결과임을 명확히 표시한다. 늦게 도착한 응답이 새 입력의 결과처럼 보이지 않게 한다.
- [x] BUY/WAIT/SKIP의 한국어 label과 confidence 백분율을 보여준다. “모델의 판단 신뢰도이며 구매 만족 확률은 아닙니다”라는 짧은 안내를 제공한다.
- [x] 360px 모바일부터 데스크톱까지 가로 넘침 없이 구성한다.
- [x] label, 키보드 탐색, focus, 충분한 대비, loading과 결과 알림을 확인한다. 색상만으로 결과를 구분하지 않는다.

### 완료 기준
실제 API 계약으로 모든 화면 상태가 작동하고 로그인·히스토리·상품 추천 등 제외 기능이 없다.

### 작업 결과
360px 모바일 단일 열과 데스크톱 가운데 한 열을 확인했다. 직접 입력/예시 전환, 5개 입력 예시, 여유자금 슬라이더와 숫자 입력, 6개 필드, loading/error/retry, 실제 결과와 조건 요약을 구현했다. 프로덕션 화면에서 노트북 예시→Jev WAIT 결과→입력 변경 시 이전 결과 제거를 확인했다. 55개 테스트, lint, typecheck, build 통과. 독립 리뷰 UI-1~UI-6 수정 후 PASS.

## 05. 통합 및 핵심 QA
상태: DONE — 25회 사전 등록 평가, 통합 검증과 독립 리뷰 PASS
선행 조건: 03, 04
담당: 메인 + QA

- [x] 입력 → 서버 API → Vercel AI Gateway → Jev → 결과 화면까지 실제 호출로 확인한다.
- [x] 정상 성공, 잘못된 JSON/입력, 키 누락, upstream 401/429/5xx, timeout, 잘못된 응답을 테스트한다. 외부 오류는 테스트용 fixture로 재현한다.
- [x] confidence의 NaN/무한대/범위 밖 값과 알 수 없는 decision을 거절하는지 확인한다.
- [x] 프리셋 적용, 중복 제출 방지, 입력 변경 후 결과 처리, 오류 후 재시도를 확인한다.
- [x] 모바일 360px/390px 및 데스크톱 화면, 키보드 조작을 점검한다.
- [x] 브라우저 번들·네트워크 응답에 API 키가 없는지 확인한다. 키 값을 출력하지 않는다.
- [x] `test`, `typecheck`, `lint`, `build`를 실행하고 프로덕션 서버에서도 핵심 동작을 확인한다.
- [x] 실제 호출 검증과 모의 테스트를 분리해 기록한다. 주관적 구매 판단의 특정 선택지를 정답으로 고정하지 않는다.

- [x] 20~30개 구매 상황으로 기준 일치와 반복 호출 일관성을 확인한다. 가격·예산·필요도만 바꾼 비교 사례를 포함한다. 주관적인 사례는 허용 선택지를 정의하고 이를 객관적 구매 정답률로 보고하지 않는다.
- [x] confidence를 구매 만족 확률이나 정답률로 표현하지 않는지 확인한다. Vercel 경유와 직접 연결의 품질 동등성은 실측 전 보장하지 않는다.

### 완료 기준
핵심 검사 통과와 실제 연결 증거가 있어야 DONE으로 표시한다. 키나 네트워크 때문에 실행하지 못한 검사는 미검증으로 남긴다.

### 작업 결과
실제 로컬 프로덕션 route로 20개 고유 조건과 5개 반복, 총 25회 Gateway 경유 Jev 요청을 실행했다. 유효 응답 25/25, 사전 허용 선택지 일치 19/20, 반복 choice 일치 5/5. S2는 WAIT를 허용한 사전 기준과 달리 BUY를 반환했다(confidence 0.25). 상세 사례·결과·한계는 `qa/`에 기록했다. 무효 JSON·입력 400과 정상 200을 확인했다. `npm test` 55개, typecheck, lint, build 통과. 클라이언트 정적 파일 11개에 실제 키가 없고 `.env.local`은 git 추적에서 제외됐다. 독립 리뷰 PASS. 이는 객관적 구매 정답률이 아니다.

## 06. README 및 배포 준비
상태: DONE — README·비밀 파일 제외·배포 설정 점검 및 독립 리뷰 PASS
선행 조건: 05
담당: 메인

- [x] README에 목적·범위·지원 Node·설치·환경변수·개발/테스트/빌드/실행 방법을 적는다.
- [x] Vercel 경유 Jev가 현재 기본 경로이며 직접 연결은 보류임을 README에 명시한다. 공식 Gateway/TypeSafe 문서, 확인 버전, 서버 `AI_GATEWAY_API_KEY` 설정, confidence 의미를 설명한다.
- [x] Gateway 가격·크레딧·계정 제한을 확인하고 무료를 전제로 배포하지 않는다.
- [x] 프리셋은 예시 가격이며 실제 상품 정보가 아님을 적는다.
- [x] 키 누락·인증 실패·429·timeout의 해결 방법과 알려진 제한을 기록한다.
- [x] Next.js 서버 기능을 지원하는 배포 환경 기준으로 서버 환경변수, build/start 명령과 설정 절차를 문서화한다. 정적 export로 처리하지 않는다.
- [x] 로그인 없는 API의 공개 배포 전 호출 제한/비용 통제가 필요한지 점검하고 호스팅 플랫폼의 간단한 보호 설정을 안내한다. 별도 DB 인프라는 추가하지 않는다.
- [x] 비밀 파일·생성 파일이 커밋 대상에 없는지 최종 확인한다.
- [x] 단계별 로컬 커밋 규칙을 준수했는지 확인하고 원격 push 여부는 별도로 결정한다. 실제 공개 배포와 새 유료 서비스 생성은 별도 지시 없이 수행하지 않는다.
- [x] 완료 항목, 검증 결과, 실행 방법, 남은 제약을 최종 보고한다.

### 완료 기준
처음 보는 사람이 README로 로컬 실행과 배포 설정을 재현할 수 있다. 배포 준비 완료를 실제 배포 완료로 보고하지 않는다.

### 작업 결과
README를 구현 상태에 맞게 갱신했다. Node 24.4+, npm 명령, `AI_GATEWAY_API_KEY` 서버 설정, Gateway 경유 Jev, 503/429/timeout, confidence와 예시 가격, Next 서버 배포·호출/비용 제한을 짧게 설명했다. `.vercel/`을 gitignore에 추가했다. `.env.local`, `.next`, `.vercel` 제외를 확인했고 독립 리뷰 PASS. 실제 공개 배포와 원격 push는 수행하지 않았다.

## 07. 24시간 자동 실행 운영 계획
상태: DONE — 00~06 및 07 최종 기록의 독립 리뷰 PASS. 전체 24시간 기준의 시작 시각은 미기록이고 별도 예약 실행은 설정하지 않음
담당: 메인
...
### 실행 기록
- 시작: 2026-09-22 KST 사용자 재개 지시 후. 정확한 최초 실행 시각은 별도 기록되지 않았다. 최초 완료 커밋 `e378d7f`는 22:24 KST다.
- 완료 확인: 2026-09-22 23:25 KST, 00~06 완료 커밋 및 clean worktree 확인. 최초 완료 커밋부터 약 1시간 뒤다. 최초 사용자 실행 지시부터의 총 소요 시간은 시작 시각 미기록으로 확인할 수 없다.
- 현재 단계: 00~07 DONE. 공개 배포·원격 push는 미실행.
- 막힘: 없음. Jev 실제 호출 25/25 유효, 단 S2의 사전 기준 불일치는 QA 보고서에 남겼다.
- 다음 행동: 사용자가 원할 때 원격 push·실제 Vercel 배포. 별도 예약 자동화는 생성하지 않았다. 공개 전 호출/비용 제한 설정 필요.



## 최신 UI 구현 기준
참고 이미지: [purchase-app-ui.png](../references/purchase-app-ui.png). 이 기준은 04 화면 구현에 적용됐고 해당 단계는 완료됐다.
